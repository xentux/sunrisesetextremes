import unittest
from skyfield import api, almanac, searchlib
from skyfield.searchlib import find_minima, find_maxima
from pytz import timezone
from math import modf

ts = api.load.timescale()
load = api.Loader('/var/data'). # shared BSP file directory 
planets = load('de421.bsp')  # load JPL Ephemeris, DE421 is most accurate for AD 1900 to 2050
earth = planets['earth']
sun = planets['sun']
mars = planets['mars']

def risesetextremes(lat, lon, tzstr, startYear=2020, years=1, verbose=False):
    ...
    for y in range(startYear, startYear+years):
        result[str(y)] = dict()
        for x in ['rise', 'set']:
           result[str(y)][f"{x}_earliest"] = data[str(y)][f"{x}s"][data[str(y)][f"{x}_hr"].index(min(data[str(y)][f"{x}_hr"]))]
           result[str(y)][f"{x}_latest"] = data[str(y)][f"{x}s"][data[str(y)][f"{x}_hr"].index(max(data[str(y)][f"{x}_hr"]))]
        for x in ['short', 'long']:
            if x == 'short':
                f = min
            else:
...
if __name__ == '__main__':
    # 35.7796° N, 78.6382° W
    jkl, data = risesetextremes('35.78', '-78.64', "US/Eastern")

    for x in ['rise', 'set']:
        print (x)
        for y in ['earliest', 'latest']:
            print ("%20s: %s" % (y, jkl['2020'][f"{x}_{y}"]))
